...
class TCPClient(QWidget):
    def __init__(self):
        ...
        self.connect_button = QPushButton('Connect', clicked=self.connect_to_server)
        self.connect_button.setFont(QFont('Microsoft YaHei', 9))
        self.disconnect_button = QPushButton('Disconnect', clicked=self.disconnect_from_server)
        self.disconnect_button.setFont(QFont('Microsoft YaHei', 9))

        self.send_button = QPushButton('Send', clicked=self.send_data)
        self.send_button.setFont(QFont('Microsoft YaHei', 9))

        self.show_photo_button = QPushButton('Show Photo', clicked=self.show_photo_viewer)
        self.show_photo_button.setFont(QFont('Microsoft YaHei', 9))

        self.clear_data_button = QPushButton('Clear Data', clicked=self.clear_data)
        ...
        self.file_button = QPushButton('Select file', clicked=self.select_file)
        self.file_button.setFont(QFont('Microsoft YaHei', 9))
        self.start_button = QPushButton('Download program', clicked=self.start)
        ...
        button_layout.addWidget(self.show_photo_button)
    ...
    def receive_data(self):
        if self.send_format_ascii:
            ...
            receive_current_time = datetime.now().time().strftime("%H:%M:%S.%f")[:-3]
            data = ' '.join(hex_data[i:i + 2] for i in range(0, len(hex_data), 2))
    ...
    def send_data(self):
        if self.tcp_socket.state() == QTcpSocket.ConnectedState:
            ...
            if self.send_format_ascii:
                ...
                send_current_time = datetime.now().time().strftime("%H:%M:%S.%f")[:-3]
                send_message = f"[{send_current_time}] Send:{data}"
                self.view_textedit.append(send_message)
