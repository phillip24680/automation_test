...
class SerialTool(QMainWindow, Ui_MainWindow):
	...
	def StartComActivated(self):
		if self.l_serial.isOpen():
			...
			self.l_serial.close()
			self.Button_Onoff_com.setText("open port")
			...
			# 检查串口参数的合法性
			if not self.checkSerialParam():
				QMessageBox.critical(self, 'pycom', 'Serial port parameter setting error!')
				return

			self.l_serial.open()
			self.Button_Onoff_com.setText("close port")
	...
	def checkSerialParam(self):
		try:
			baudrate = int(self.Combo_Baudrate.currentText())
			bytesize = int(self.Combo_Data_bit.currentText())
			stopbits = int(self.Combo_Stop_bit.currentText())
			parity = self.Combo_Parity.currentText()

			serial.Serial(
				port=self.l_serial.port,
				baudrate=baudrate,
				bytesize=bytesize,
				stopbits=stopbits,
				parity=parity
	...
	def UartSend(self):
		...
		if not self.l_serial.isOpen():
			QMessageBox.critical(self, 'pycom', 'Please open the serial port first!')
			return
		...
		if self.Box_Hex_send.checkState():
			...
			while InputStr != '':
				try:
					...
					QMessageBox.critical(self, 'pycom', 'Please enter hexadecimal data separated by spaces!')
					return None
